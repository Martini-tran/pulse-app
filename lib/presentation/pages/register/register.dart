...
class _RegisterPageState extends State<RegisterPage> with TickerProviderStateMixin {
  ...
  String? _validateConfirmPassword(String? value) {
    ...
    if(value != _passwordController.text){  // ✅ 正确：使用 .text 获取文本内容
      return '密码不一致';
    }
    return null;
  }
  ...
  void _handleRegister() async {
    ...
    try {
      logger.info("开始注册请求");
      ApiResponse<LoginResponseDto> apiResponse = await UserApiService.register(
      ...
      if (apiResponse.success && apiResponse.code == 200 && apiResponse.data != null) {
        ...
        switch (userInfo.status) {
          case '1000':
            logger.info("注册成功，用户状态: ${userInfo.status}, 跳转到引导页");
            ToastUtils.success('注册成功，开始您的旅程！');
            context.go("/guide_1");
            break;
          case '2000':
            logger.info("注册成功，用户状态: ${userInfo.status}, 跳转到主页");
